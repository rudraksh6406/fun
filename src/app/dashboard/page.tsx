...
function MoodIcon({ mood, hue, active }: { mood: string; hue: number; active: boolean }) {
  const color = `hsla(${hue}, 60%, ${active ? 55 : 65}%, ${active ? 0.8 : 0.3})`;
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="section-light py-20 md:py-32 px-8 md:px-12">
        <div className="max-w-[1400px] mx-auto grid lg:grid-cols-[420px_1fr] gap-16 lg:gap-24">
          {/* Controls */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            <div className="lg:sticky lg:top-32 space-y-12">

              {/* Time — arc visualization */}
              <div>
                ...
                <div className="relative flex items-center gap-6">
                  ...
                  <div className="relative w-24 h-24 shrink-0">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <motion.span
                        ...
                        className="text-2xl font-light text-[#0c0c0c]"
                      >
                        {time}
                      </motion.span>
                    </div>
                  ...
                  <div className="flex-1">
                    <p className="body-sm text-black/40 mb-3">minutes available</p>
                    <input type="range" min={15} max={480} step={15} value={time} onChange={(e) => setTime(Number(e.target.value))}
                      className="w-full h-[2px] appearance-none bg-purple-200/30 outline-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-purple-500 [&::-webkit-slider-thumb]:shadow-[0_0_10px_rgba(168,85,247,0.4)]" />
                  </div>
                </div>
              </div>

              {/* People — animated avatar dots */}
              <div>
                <span className="label block mb-5" style={{ color: "rgba(168,85,247,0.4)" }}>People</span>
                <div className="flex items-center gap-5">
                  <motion.button
                    onClick={() => setPeople(Math.max(1, people - 1))}
                    whileTap={{ scale: 0.85 }}
                    className="w-10 h-10 rounded-full border border-purple-300/20 flex items-center justify-center text-purple-400/50 hover:border-purple-400/40 hover:text-purple-500 transition-all hover:bg-purple-50/50"
                  >
                    &minus;
                  </motion.button>
                      ...
                      {[...Array(Math.min(people, 12))].map((_, i) => {
                        ...
                        return (
                          <motion.div
                            ...
                            style={{
                              background: `hsla(${hue}, 50%, 65%, 0.12)`,
                              color: `hsla(${hue}, 50%, 55%, 0.8)`,
                              border: `1px solid hsla(${hue}, 50%, 65%, 0.2)`,
                            }}
                          >
                            {i + 1}
                          </motion.div>
              ...
              {/* Mood — glowing orb cards */}
              <div>
                <span className="label block mb-5" style={{ color: "rgba(168,85,247,0.4)" }}>Mood</span>
                  ...
                  {moods.map((m) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.93 }}
                        className="relative py-4 px-3 rounded-sm text-center transition-all duration-300 overflow-hidden"
                        style={{
                          background: isActive ? `hsla(${m.hue}, 50%, 60%, 0.08)` : "transparent",
                          border: `1px solid ${isActive ? `hsla(${m.hue}, 50%, 60%, 0.3)` : "rgba(168,85,247,0.1)"}`,
                        }}
                      >
                        {isActive && (
                          <motion.div
                            ...
                            style={{ background: `radial-gradient(circle at center, hsla(${m.hue}, 60%, 60%, 0.08), transparent 70%)` }}
                            transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
                          />
                        )}
                        ...
                        <span
                          className="text-[10px] tracking-[0.1em] uppercase font-medium relative z-10"
                          style={{ color: isActive ? `hsla(${m.hue}, 50%, 45%, 0.9)` : "rgba(168,85,247,0.35)" }}
                        >
                          {m.name}
                        </span>
                      </motion.button>
              ...
              {/* Materials */}
              <div>
                <span className="label block mb-4" style={{ color: "rgba(168,85,247,0.4)" }}>Materials</span>
                    ...
                    >
                      {materials.map((m) => (
                        <motion.span
                          ...
                          className="chip-dark active cursor-pointer text-[11px]"
                        >
                          {m} &times;
                        </motion.span>
                      ))}
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3">
                  {materialSuggestions.filter((m) => !materials.includes(m)).slice(0, 5).map((m) => (
                    <button key={m} onClick={() => addMaterial(m)}
                      className="text-[11px] text-purple-400/30 hover:text-purple-500/60 transition-colors">+ {m}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="btn btn-light w-full justify-center opacity-50 hover:opacity-100 transition-opacity"
                >
                  Surprise Me
                </motion.button>
              </div>
          ...
          <div>
                ...
                >
                  ...
                  <div className="relative w-28 h-28">
                    ...
                    <motion.div
                      ...
                      className="absolute inset-9 rounded-full bg-purple-400/10"
                    />
                    {/* Orbiting dots */}
                    {[0, 1, 2].map((d) => (
                      <motion.div
            ...
            {!isLoading && recommendations.length > 0 && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-14">
                  {recommendations.map((a, i) => <ActivityCard key={a.id} activity={a} index={i} variant="light" />)}
                </div>
              </motion.div>
            )}
            ...
            {!isLoading && generatedIdeas.length > 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-14">
                  {generatedIdeas.map((a, i) => <ActivityCard key={a.id} activity={a} index={i} variant="light" />)}
                </div>
              </motion.div>
            )}
            ...
            {!isLoading && !hasSearched && (
              <div className="py-20 flex flex-col items-center">
                {/* Animated idle state */}
                <motion.div className="relative w-32 h-32 mb-8">
                  <motion.div
                    animate={{ rotate: 360 }}
                    transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                    className="absolute inset-0 rounded-full border border-purple-400/[0.08]"
                  />
                  <motion.div
                    animate={{ rotate: -360 }}
                    transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                    className="absolute inset-4 rounded-full border border-cyan-400/[0.06]"
                  />
                  <motion.div
                    animate={{ rotate: 360 }}
                    transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
                    className="absolute inset-8 rounded-full border border-pink-400/[0.08]"
                  />
                  <motion.div
                ...
                <p className="title" style={{ color: "rgba(168,85,247,0.2)" }}>Set your preferences,</p>
                <p className="title" style={{ color: "rgba(168,85,247,0.2)" }}>then let the AI cook.</p>
              </div>
            )}
          </div>
        </div>
